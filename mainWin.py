...
from assets.constants.Constants import BACKGROUND_COLOR, HEIGHT_ROOT_LINUX, HEIGHT_ROOT_WINDOWS, WIDTH_ROOT_WINDOWS
...
class MainWindow:
    ...
    def __init__(self, root):
        self.mask = False
        containersSize = {
            "width": (WIDTH_ROOT_WINDOWS / 2) - 2,
            "height": HEIGHT_ROOT_WINDOWS,
        }
        image_path = base_path / Path("assets/Images/Logo.png")

        root.geometry("{0}x{1}".format(WIDTH_ROOT_WINDOWS, HEIGHT_ROOT_WINDOWS))
        ...
        # region Hotkeys
        keyboard.add_hotkey("ctrl+1", self.get_rg)
        keyboard.add_hotkey("ctrl+2", self.get_cpf)
        keyboard.add_hotkey("ctrl+3", self.get_cnpj)
